...
The checks that requires manual steps to determine whether a system’s configured state is as expected is skipped in the scan. Manual recommendations are equally important to automated and this should be validated using the assessment steps provided in [ CIS Amazon Linux 2 Benchmark ](https://www.cisecurity.org/benchmark/amazon_linux).
...
_Potential Operation Impact - Recommendation wasn't applied because it would have a negative effect on the service._
