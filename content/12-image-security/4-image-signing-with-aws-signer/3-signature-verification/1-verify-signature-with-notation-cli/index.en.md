...
As mentioned earlier in the Notation install, the _trustpolicy_ was found in the notation directory tree. However, the _trustpolicy_ installed at `~/.config/notation/trustpolicy.json` by the installer is an **empty file**.
...
Let us create a valid _trustpolicy_ document using below command.
...
The above Notation _trustpolicy_ document configures Notation to verify container image signatures using the AWS Signer signing profile we used in the previous section to sign the container image. As you can see, the _trustpolicy_ configures the following items:
...
### Next, we need to configure the above _trustpolicy_ document with Notation to use for verifying the signatures.
...
The above command asks for a user input **y** or **N**. Type **y** and press _Enter_.
...
We can also see the imported and configured _trustpolicy_ with the `notation policy show` command as below.
...
Congratulations !!! You have successfully completed the Module on signing the container image and verifying the signature using the Notation CLI.
